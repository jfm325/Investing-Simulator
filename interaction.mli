...
(** The type [invest] represents the purchase/sale of shares that can be
    ...
    the second element is the #shares to be purchased/sold in the transaction
    ...
    (** [find_Stock list symb] checks if the stock with ticker symbol 
        [symb] is present or not i.e if it is present in [list] which is a Stock.t 
        list. If symb is found in list, then Stock.t with ticker_symbol symb is 
        returned. Otherwise raises Not_found.
        *)
    val find_Stock : Stock.t list -> Stock.ticker_symbol -> Stock.t 
    
    
    (** [print_stocks s_lst] prints the stock and its current price. *)
    val print_stocks : Stock.t list -> unit 
    ...
    (** [view com u] executes the parsed command that was the input of user u and 
    ...
    - [view Cash] : displays current cash of user 
    
    - [view Networth] : displays current networth of user 
    
    - [view Buy ["COKE" ; "50"]] : changes portfolio of user by purchasing 50 stocks
    of "COKE" if exists in the market.
    
    - [view Sell ["COKE" ; "50"]] : changes portfolio of user by selling 50 stocks
    of "COKE" if exists in the market.
